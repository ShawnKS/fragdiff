...
from utils.brics_rec import *
from utils.recap_rec import *
from utils.junc_dec import *
...
def get_torsion_angles_brics(mol):
    torsions_list = []
    G = nx.Graph()
...
def get_torsion_angles_recap(mol):
    torsions_list = []
    G = nx.Graph()
    for i, atom in enumerate(mol.GetAtoms()):
        G.add_node(i)
...
def get_torsion_angles_junc(mol):
    torsions_list = []
    # G = nx.Graph()
    # for i, atom in enumerate(mol.GetAtoms()):
    #     G.add_node(i)
    ...
    for e in ring_edges:
        side_atoms = find_subring_atoms(mol, e)
        other_side_atoms = get_other_atoms(mol, side_atoms)
        neighbors_e0 = find_neighbors(mol, e[0], e[1])
        n0 = [n for n in neighbors_e0 if n in side_atoms]
        neighbors_e1 = find_neighbors(mol, e[1], e[0])
        n1 = [n for n in neighbors_e1 if n in other_side_atoms]
        torsions_list.append(
            (n0[0], e[0], e[1], n1[0])
        )
    # torsions_list_conn = []
...
def fast_rmsd(mol1, mol2, conf1=0, conf2=0):
    A = mol1.GetConformers()[conf1].GetPositions()
    B = mol2.GetConformers()[conf2].GetPositions()
    A -= rmsd.centroid(A)
    B -= rmsd.centroid(B)
    U = rmsd.kabsch(A, B)
    A = np.dot(A, U)
