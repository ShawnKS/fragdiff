...
if __name__ == '__main__':
    ...
    sub_dir = args.dataset + '_' + args.dec + '_bs' + str(args.batch_size)
    cache = args.cache
    if args.std_pickles:
        sub_dir = sub_dir + '_std'
        # + args.std_pickles.split('/')[-1]
        if cache: args.cache = args.cache + '_std'
    else:
        sub_dir = sub_dir + '_nostd'
        if cache: args.cache = args.cache + '_nostd'
    if args.aug:
        # sub_dir = sub_dir + '_aug'
        sub_dir = sub_dir + '/aug'
        if cache: args.cache = args.cache + '_aug'
    if args.rigid:
        sub_dir = sub_dir + '_rigid'
        if cache: args.cache = args.cache + '_rigid'
    if args.limit_train_mols:
        sub_dir = sub_dir + '_' + str(args.limit_train_mols)
        #  + 'nepochs_' + str(args.n_epochs)
        if args.z:
            if cache: args.cache = args.cache + '_' + str(args.limit_train_mols)
    args.log_dir = os.path.join(parent_dir, sub_dir)
    if args.z:
        args.log_dir = os.path.join(args.log_dir, str(args.z)+ 'nepochs_' + str(args.n_epochs) )
        if cache: args.cache = args.cache + '_z' + str(args.z)
    # args.log_dir = os.path.join(parent_dir, sub_dir)
