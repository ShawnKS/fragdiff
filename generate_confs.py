...
def embed_func(mol, numConfs):
    AllChem.EmbedMultipleConfs(mol, numConfs=numConfs, numThreads=20)
    return mol
...
def sample_confs(raw_smi, n_confs, smi):
    print(raw_smi)
    if args.seed_confs:
        mol, data = get_seed(raw_smi, seed_confs=seed_confs, dataset=args.dataset, dec=args.dec)
    elif args.seed_mols:
        mol, data = get_seed(smi, seed_confs=seed_confs, dataset=args.dataset, dec=args.dec)
        mol.RemoveAllConformers()
    else:
        mol, data = get_seed(smi, dataset=args.dataset, dec=args.dec)
    if not mol:
